...
// --- NEW MAPPING FOR DYNAMIC NAVIGATION ---
/**
 * Generates a screen name based on the selected role and action type (Login/Register).
 * E.g., ('Middleman/Wholesaler', 'Register') -> 'WholesalerRegisterScreen'
 */
const getScreenName = (role, type) => {
  // Clean up the role name: remove spaces and extra text (like "/Wholesaler")
  // 'Middleman/Wholesaler' becomes 'Middleman'
  // 'Transport Partner' becomes 'TransportPartner'
...
const LanguageSelectScreen = ({ navigation }) => {
  ...
  const roles = ['Farmer', 'Middleman/Wholesaler', 'Transport Partner', 'Retailer'];
  ...
  const handleLoginPress = () => {
    if (!selectedRole) {
      Alert.alert('Selection Required', 'Please select your role before logging in.');
      return;
    }
    const screenName = getScreenName(selectedRole, 'Login');
    
    if (screenName) {
        // Navigate to the dynamic Login screen, passing current selections
        console.log(`Navigating to Login Screen: ${screenName}`);
        navigation.navigate(screenName, { role: selectedRole, language: selectedLanguage });
    } else {
        Alert.alert('Error', 'Invalid role selected for login.');
    }
  };

  const handleRegisterPress = () => {
    if (!selectedRole) {
      Alert.alert('Selection Required', 'Please select your role before registering.');
      return;
    }
